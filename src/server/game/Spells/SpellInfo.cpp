...
SpellCastResult SpellInfo::CheckTarget(Unit * caster, Unit const* target, bool implicit) const
{
    if (AttributesEx & SPELL_ATTR1_CANT_TARGET_SELF && caster == target)
        return SPELL_FAILED_BAD_TARGETS;

    if (AttributesEx & SPELL_ATTR1_NOT_IN_COMBAT_TARGET && target->isInCombat())
    ...
    if (AttributesEx3 & SPELL_ATTR3_REQUIRE_DEAD_TARGET && !(!target->isAlive() && target->HasAuraType(SPELL_AURA_GHOST)))
       return SPELL_FAILED_TARGET_NOT_GHOST;

    // check this flag only for implicit targets (chain and area), allow to explicitly target units for spells like Shield of Righteousness
    if (implicit && AttributesEx6 & SPELL_ATTR6_IGNORE_CROWD_CONTROL_TARGETS && !target->CanFreeMove())
       return SPELL_FAILED_BAD_TARGETS;
