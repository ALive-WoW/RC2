...
public:
        ...
        bool OnGossipHello(Player* player, Creature* pCreature)
    {
        ...
        {
            player->ADD_GOSSIP_ITEM(0, "Begin Battle", 631, 1001);
            player->SEND_GOSSIP_MENU(player->GetGossipTextId(pCreature + 1), pCreature->GetGUID());
            return true;
        }
        return false;
    }
    ...
    struct npc_apothecary_baxterAI : public ScriptedAI
    {
        ...
        {
            Creature* Hummel = me->GetCreature(*me, pInstance->GetData64(DATA_HUMMEL));
            switch (action)
            {
                case ACTION_INTRO_START:
                    ...
                    if (Hummel)
                    {
                        Hummel->setFaction(16);
                        Hummel->AI()->DoZoneInCombat();
                    }
                    break;
