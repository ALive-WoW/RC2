...
// UPDATE `creature_template` SET `modelid1`='10701', `modelid2`='10701', `ScriptName`='npc_tobender_geist' WHERE `entry`='36701';
// UPDATE `creature_template` SET `scale`='3' WHERE `entry`='36701';
// UPDATE `creature_template` SET `speed_walk`='0.2', `speed_run`='0.2' WHERE `entry` IN ('37799', '39284', '39285', '39286');
...
struct Position MovePos[]=
{
    ...
    {490.110f, -2124.98f, 1040.860f, 0.0f}, // move tirion frostmourne 490.110, -2124.98, 1040.860
...
{
    {520.0f, -2524.0f, 1051.0f, 0.0f}, //Place where player is teleported to - 520.0, -2524.0, 1051.0
...
void RemoveAllPassengers(Unit* me)
{
	// Fix Passenger Drop
	me->SetVisible(false);
	me->UnitKill(me);
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                ...
                switch(id)
                {
                    case POINT_PLATFORM_CENTER:
                    {
						...
						events.ScheduleEvent(EVENT_TOBENDER_GEIST, 21000, 0, curPhase);
            ...
            void DoAction(const int32 action)
            {
                switch(action)
                {
                    ...
                    case ACTION_CANCEL_ALL_TRANSITION_EVENTS:
                    {
						...
						events.CancelEvent(EVENT_TOBENDER_GEIST);
                        break;
                    }
                    ...
                    case ACTION_PHASE_SWITCH_2:
                    {
						...
						void npc_vile_spirit(Unit* caster)
			{
                ...
                if (instance)
                {
					...
					if (maxSummoned == 8)
					{
						me->SummonCreature(NPC_VILE_SPIRIT, me->GetPositionX()+urand(1,10), me->GetPositionY()+urand(1,10), me->GetPositionZ());
						me->SummonCreature(NPC_VILE_SPIRIT, me->GetPositionX()+urand(1,10), me->GetPositionY()+urand(1,10), me->GetPositionZ());
					    me->SummonCreature(NPC_VILE_SPIRIT, me->GetPositionX()+urand(1,10), me->GetPositionY()+urand(1,10), me->GetPositionZ());
						me->SummonCreature(NPC_VILE_SPIRIT, me->GetPositionX()+urand(1,10), me->GetPositionY()+urand(1,10), me->GetPositionZ());
						me->SummonCreature(NPC_VILE_SPIRIT, me->GetPositionX()+urand(1,10), me->GetPositionY()+urand(1,10), me->GetPositionZ());
                    ...
                    switch (GetPhase(events))
                    {
                        ...
                        case PHASE_4_TRANSITION:
                        {
                            switch (eventId)
                            {
								...
								case EVENT_TOBENDER_GEIST:
								{
									//if(isgeist == false)
									//{

									Player* randomPlayer = SelectRandomAttackablePlayerInTheMap(me->GetMap());
									randomplayerguid = randomPlayer->GetGUID();
									randomPlayer->CastSpell(randomPlayer, 69201);
									//isgeist = true;
									events.ScheduleEvent(EVENT_TOBENDER_GEIST, 21000, 0, GetPhase(events));
									//}

									/*if(isgeist == true)
									{
										Player* randomPlayer = me->GetPlayer(*me, randomplayerguid);
										if(Creature* tobendergeist = randomPlayer->FindNearestCreature(36701, 15.0f, true))
										{
											randomPlayer->CastSpell(tobendergeist, 57507);
											tobendergeist->SetFloatValue(OBJECT_FIELD_SCALE_X, 2);
											isgeist = false;
											events.ScheduleEvent(EVENT_TOBENDER_GEIST, 21000, 0, GetPhase(events));
											break;
										}
                        ...
                        case PHASE_3:
                        {
                            switch (eventId)
                            {
                                case EVENT_SUMMON_VAL_KYR_SHADOWGUARD:
                                {
									...
									//me->SummonCreature(NPC_VALKYR, 487.970734f, -2140.089355f, 1053.557983f);
									//me->SummonCreature(NPC_VALKYR, 504.326355f, -2147.273926f, 1057.862915f);
                        ...
                        switch(uiStage)
                        {
                            ...
                            case 10:
                            {
                                if (uiTirionGUID)
								{
                                    if (Creature* tirion = Unit::GetCreature(*me, uiTirionGUID))
									{
										tirion->RemoveAura(SPELL_ICEBLOCK_TRIGGER);
                            ...
                            case 11:
                            {
                                if (uiTirionGUID)
                                {
                                    if (Creature* tirion = Unit::GetCreature(*me, uiTirionGUID))
                                    {
                                        tirion->GetMotionMaster()->MoveJump(510.112854f, -2124.355225f, 1040.859985f, 10.0f, 15.0f);
                            ...
                            case 12:
                            {
                                if (Creature* tirion = Unit::GetCreature(*me, uiTirionGUID))
								{
									...
									tirion->NearTeleportTo(510.112854f, -2124.355225f, 1040.859985f, 3.0f,false);
									uiEndingTimer = 1000;
									break;
								}
                            }
                            ...
                            case 19:
                            {
                                if (Creature* father = me->FindNearestCreature(NPC_TERENAS_MENETHIL, 25.0f, true))
                                {
									...
									for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
									{
										//itr->getSource()->SetHealth(itr->getSource()->GetMaxHealth());
										father->CastSpell(itr->getSource(),SPELL_REVIVE_EFFECT);
									}
                ...
                if (uiIntroTimer <= diff)
                {
                    switch(uiStage)
                    {
                        ...
                        case 8:
                            ...
                            me->GetMotionMaster()->MovePoint(0, 482.451874f, -2125.059814f, 1040.859985f);
                            uiIntroTimer = 1800;
                            break;
                        case 9:
                            if(Creature* lich = Unit::GetCreature(*me, uiLichKingGUID))
                                //lich->CastSpell(me, SPELL_ICEBLOCK_TRIGGER, true);
							    me->AddAura(SPELL_ICEBLOCK_TRIGGER, me);
                            uiIntroTimer = 2000;
                            break;
                        case 10:
                        {
...
class npc_valkyr_shadowguard : public CreatureScript
{
    public:
        ...
        struct npc_valkyr_shadowguardAI : public BossAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
				//me->MonsterYell("Ja ich bin die Valkyr und hab das richtige script!", LANG_UNIVERSAL, 0);
				if(!me->getVictim())
				{
					if (ideltimer <= diff  && bCanCast == false)
					{
						Map::PlayerList const &players = me->GetMap()->GetPlayers();
						for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
						{
						...
						uiLichKingGUID = instance->GetData64(DATA_THE_LICH_KING);
						if (Creature* lichking = Unit::GetCreature(*me,uiLichKingGUID))
						{
							random = urand(2, maxrandom+1);
							std::list<HostileReference*>& threatList = lichking->getThreatManager().getThreatList();
							std::vector<Unit*> eliteList;
							for (std::list<HostileReference*>::const_iterator itr = threatList.begin(); itr != threatList.end(); ++itr)
							{
								if(curend == random)
								{
									targetplayer = (*itr)->getUnitGuid();
									m_victim = Unit::GetUnit(*me, targetplayer);
									BossAI::AttackStart(m_victim);
									me->SelectVictim();
									bCanCast = true;
								}else{
									curend++;
				...
				if(bCanCast == true)
				{
					if (Unit* m_victim = Unit::GetUnit(*me, targetplayer))
					{
						DoCast(m_victim, SPELL_VALKYR_CHARGE, true);

						if (me->IsWithinDistInMap(m_victim, 7))
						{
							me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
							if (m_victim->GetTypeId() == TYPEID_PLAYER)
								m_victim->CastSpell(me, SPELL_VALKYR_GRAB_PLAYER, true);
						}
					}
				}
			}
...
class npc_tobender_geist: public CreatureScript
{
    public:
        npc_tobender_geist(): CreatureScript("npc_tobender_geist") { }

        struct npc_tobender_geistAI: public ScriptedAI
        {
            npc_tobender_geistAI(Creature* creature): ScriptedAI(creature)
            {
            }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return GetIcecrownCitadelAI<npc_tobender_geistAI>(creature);
    }
};
        ...
        class spell_lich_king_infection_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                if (!GetTarget()->isAlive() || GetTarget()->GetHealthPct() >= 90)
                {
					...
					if (GetTarget()->HasAura(70541))
					{
						GetTarget()->RemoveAurasDueToSpell(70541);
					}
					else if (GetTarget()->HasAura(73779))
					{
						GetTarget()->RemoveAurasDueToSpell(73779);
					}
					else if (GetTarget()->HasAura(73780))
					{
						GetTarget()->RemoveAurasDueToSpell(73780);
					}
					else if (GetTarget()->HasAura(73781))
					{
						GetTarget()->RemoveAurasDueToSpell(73781);
					}
                }
            }
        ...
        class spell_lich_king_valkyr_summon_AuraScript : public AuraScript
        {
            ...
            void HandleTriggerSpell(AuraEffect const* aurEff)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    //uint32 triggerSpellId = 74361;/*GetSpellProto()->EffectTriggerSpell[aurEff->GetEffIndex()];*/
					...
					caster->SummonCreature(NPC_VALKYR,randomPos.GetPositionX(),randomPos.GetPositionY(),randomPos.GetPositionZ());
                }
            }
